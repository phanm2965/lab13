...
public class FamilyTree
{
	...
	private void addLine(String line) throws TreeException
	{
		...
		else
		{
			parentNode = root.getNodeWithName(parent);  //There's a method in Node that searches for a named node. 
			 if(parentNode == null)
				 throw new TreeException("no parent node found");   //If the parent node wasn't found, there must have been something wrong in the data file. Throw an exception.
		}
	...
	TreeNode getMostRecentCommonAncestor(String name1, String name2) throws TreeException
	{
		...
		if (node1 == null)
			throw new TreeException("no node with " + name1 + "found.");
		TreeNode node2 = root.getNodeWithName(name2);		// node whose name is name2
		if (node2 == null)
			throw new TreeException("no node with " + name2 + " found.");
